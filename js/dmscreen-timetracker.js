								...
								entries: [
									{
										...
										items: encounter.data.l.items.map(it => {
											...
											if (scaledCr != null || scaledSummonLevel != null) {
												parts.push(
													"", // Display text
													scaledCr != null ? `${VeCt.HASH_SCALED}=${Parser.numberToCr(scaledCr)}` : scaledSummonLevel != null ? `${VeCt.HASH_SCALED_SUMMON}=${scaledSummonLevel}` : "",
												);
											}
